...
class YicaiSpider(scrapy.spiders.Spider):
    ...
    flag = 0
    def __init__(self):
        super(YicaiSpider,self).__init__()
     #   self.display = Display(visible=0, size=(800, 600))  #为了隐藏浏览器
     #    self.display.start()
        self.chromedriver = "E:\\mac\\id\\phantomjs-2.1.1-windows\\phantomjs-2.1.1-windows\\bin\\phantomjs.exe"
        # self.driver = webdriver.Chrome(self.chromedriver)              #若无display，会打开浏览器
        chromedriver = "/home/ubuntu/geckodriver"
        self.driver = webdriver.PhantomJS(executable_path=self.chromedriver)
